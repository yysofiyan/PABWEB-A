<?php
// panggil file database.php untuk koneksi ke database require_once "config/database.php";
?>
...
function  goodchars(e,  goods,  field)  { 117:		var  key,  keychar;
key  =  getkey(e);
if  (key  ==  null)  return  true; 120:
keychar  =  String.fromCharCode(key); 122:	keychar  =  keychar.toLowerCase(); 123:	goods	=    goods.toLowerCase(); 124:
//  check  goodkeys
if  (goods.indexOf(keychar)  !=  -1)
return  true;
//  control  keys
if  (  key==null  ||  key==0  ||  key==8  ||  key==9  ||  key==27  ) 130:		return  true;

if  (key  ==  13)  {
var  i;
for  (i  =  0;  i  <  field.form.elements.length;  i++) 135:		if  (field  ==  field.form.elements[i])
break;
...
//  validasi  image  dan  preview  image  sebelum  upload 146:	function  validasiFile()  {
var  fileInput	=  document.getElementById('foto'); 148:	var  filePath	=  fileInput.value;
var  fileSize	=  $('#foto')[0].files[0].size; 150:	//  tentukan  extension  yang  diperbolehkan
var  allowedExtensions  =  /(\.jpg|\.jpeg|\.png)$/i;
//  Jika  tipe  file  yang  diupload  tidak  sesuai  dengan  allowedExtensions,  tampilkan  pesan : 153:	if  (!allowedExtensions.exec(filePath))  {
...
//  jika  ukuran  file  yang  diupload  lebih  dari  sama  dengan  1 Mb 160:	else  if  (fileSize  >=  1000000)  {
...
//  Image  preview
if  (fileInput.files  &&  fileInput.files[0])  { 170:		var  reader  =  new  FileReader();
